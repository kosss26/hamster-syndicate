...
Быстрые end-to-end проверки критических сценариев дуэлей:

```bash
cd bot
php bin/smoke_duel.php
# или
composer smoke
```

Проверки API-уровня (роуты `/api/*`), запускать на dev/local с `DEV_AUTH_ENABLED=true`:

```bash
cd bot
php bin/smoke_api_duel.php --base-url=https://app.tvixx.ru
# или
SMOKE_API_BASE_URL=https://app.tvixx.ru composer smoke:api -- --base-url=https://app.tvixx.ru
```

Запуск обоих наборов подряд:

```bash
cd bot
SMOKE_API_BASE_URL=https://app.tvixx.ru ./bin/smoke_all.sh
# или
SMOKE_API_BASE_URL=https://app.tvixx.ru composer smoke:all
```

One-command серверное обновление (код + миграции + smoke + build + рестарт сервисов):
...
```bash
cd /var/www/quiz-bot
RUN_API_SMOKE=1 SMOKE_API_BASE_URL=https://app.tvixx.ru ./UPDATE_SERVER.sh
```

Отключить фронтенд smoke перед сборкой (если нужно временно):
